...
Thus $v \in span(v_1 - v_2, v_2 - v_3, v_3 - v_4, v_4)$, and $V \subseteq span(v_1 - v_2, v_2 - v_3, v_3 - v_4)$. 
...
$$v=a_1 w_1 + a_2 (w_2 - w_1) + a_3 (w_3-w_2) + ... + a_m (w_m-w_{m-1}) = (a_1-a_2)w_1 + (a_2-a_3)w_2 + ... + (a_{m-1}-a_m)w_{m-1} + a_m w_m$$. 
...
$$w=(a_1+a_2+...+a_m) v_1 + (a_2+...+a_m)v_2 + ... + a_m v_m$$. 

Thus $w \in span(v_1, ..., v_m)$, and $span(w_1, ..., w_m) \subseteq span(v_1, ..., v_m)$. 
...
Given that $v_1, v_2, v_3, v_4$ are linearly independent, we have $a=b-a=c-b=d-c=0$, $a=b=c=d=0$. 
...
$$(5a_1)v_1 + (a_2-4a_1) v_2 + a_3 v_3 + ... + a_m v_m = 0$$. 
...
$$(a_1 \lambda) v_1 + ... + (a_m \lambda) v_m = 0$$. 

Given that $v_1, ..., v_m$ are linearly independent, $a_1 \lambda =...=a_m \lambda=0$. Since $\lambda \neq 0$, this implies that $a_1= ...= a_m=0$. 
...
$$v_k+w = a_1 (v_1+w) + ... + a_{k-1} (v_{k-1} +w)$$. 

Expanding, we have 

$$(1-a_1-...-a_{k-1}) w = a_1 v_1 + ... + a_{k-1} v_{k-1}$$. 
...
Since $w \notin span(v_1,...,v_m)$, it is not a linear combination of any of the $v$'s that come before it. 
...
Thus $v_1,...,v_m$ is linearly independent.
...
Since $V=U+W$, $v=u+w, u \in U, w \in W$. 
...
Given that $V$ is a direct sum of $U$ and $W$, we have $v=u+w, u \in U, w \in W$. 
...
From Definition 1.41, $V$ is a direct sum of $V_1,...,V_n$ iff $v=u_1+...+u_n, u_1 \in V_1, u_n \in V_n$ is unique. 
